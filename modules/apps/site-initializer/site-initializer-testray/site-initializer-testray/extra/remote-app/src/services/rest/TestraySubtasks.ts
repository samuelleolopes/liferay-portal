...
class TestraySubtaskImpl extends Rest<subtaskForm, TestraySubTask> {
	...
	public async complete(subtaskId: number, id: number[], dueStatus: string) {
		const data = {
			dueStatus: SUB_TASK_STATUS['COMPLETE'],
			r_userToSubtasks_userId: Liferay.ThemeDisplay.getUserId(),
		};
		await this.fetcher.put(`/subtasks/${subtaskId}`, data);
		for (const caseResult of id) {
			const data = {
				dueStatus: dueStatus as string,
			};
			await this.fetcher.put(`/caseresults/${caseResult}`, data);
		}
	}
