		...
		<#else>
			<div class="empty-container">
				<div class="d-flex empty-result-message flex-column">
					...
					<h1 class="empty-message-title">${languageUtil.get(locale,
						"no-highlights-found-for-selected-product-capabilities", "No Highlights Found for selected
						Product Capabilities")}</h1>
					<p class="empty-message-description">${languageUtil.get(locale,
						"try-selecting-different-product-capabilities-to-view-highlights", "Try selecting different
						product capabilities to view highlights")}</p>
				</div>
			</div>
	</#if>
</div>